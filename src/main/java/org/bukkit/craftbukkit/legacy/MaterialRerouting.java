...
public class MaterialRerouting {
    ...
    private static Material transformFromBlockType(BlockType blockType, ApiVersion version) {
        if (blockType == null) {
            return null;
        }

        if (version.isOlderThan(ApiVersion.FLATTENING)) {
            return CraftLegacy.toLegacyData(blockType.asMaterial(), false).getItemType();
        }

        return blockType.asMaterial();
    }

    private static Material transformFromItemType(ItemType itemType, ApiVersion version) {
        if (itemType == null) {
            return null;
        }

        if (version.isOlderThan(ApiVersion.FLATTENING)) {
            return CraftLegacy.toLegacyData(itemType.asMaterial(), true).getItemType();
        }

        return itemType.asMaterial();
    }

    private static BlockType transformToBlockType(Material material) {
        if (material == null) {
            return null;
        }

        if (material.isLegacy()) {
            return CraftLegacy.fromLegacy(new MaterialData(material), false).asBlockType();
        }

        return material.asBlockType();
    }

    private static ItemType transformToItemType(Material material) {
        if (material == null) {
            return null;
        }

        if (material.isLegacy()) {
            return CraftLegacy.fromLegacy(new MaterialData(material), true).asItemType();
        }
    ...
    public static Material getMaterial(BlockData blockData, @InjectPluginVersion ApiVersion version) {
        return transformFromBlockType(blockData.getBlockType(), version);
    }

    public static Material getPlacementMaterial(BlockData blockData, @InjectPluginVersion ApiVersion version) {
        return transformFromItemType(blockData.getPlacementType(), version);
    }
    ...
    public static Map<DecoratedPot.Side, Material> getSherds(DecoratedPot decoratedPot, @InjectPluginVersion ApiVersion version) {
        ...
        for (Map.Entry<DecoratedPot.Side, ItemType> entry : decoratedPot.getSherds().entrySet()) {
    ...
    public static Material getMaterial(Boat.Type type, @InjectPluginVersion ApiVersion version) {
        return transformFromItemType(type.getItemType(), version);
    }

    @Deprecated
    public static Material getMaterial(FallingBlock fallingBlock, @InjectPluginVersion ApiVersion version) {
        return transformFromBlockType(fallingBlock.getBlockData().getBlockType(), version);
    }
    ...
    public static boolean addBarterMaterial(Piglin piglin, Material material) {
        return piglin.addBarterItem(transformToItemType(material));
    }

    public static boolean removeBarterMaterial(Piglin piglin, Material material) {
        return piglin.removeBarterItem(transformToItemType(material));
    }

    public static boolean addMaterialOfInterest(Piglin piglin, Material material) {
        return piglin.addItemOfInterest(transformToItemType(material));
    }

    public static boolean removeMaterialOfInterest(Piglin piglin, Material material) {
        return piglin.removeItemOfInterest(transformToItemType(material));
    }
    ...
    public static Material getSteerMaterial(Steerable steerable, @InjectPluginVersion ApiVersion version) {
        return transformFromItemType(steerable.getSteerItem(), version);
    }

    public static Material getMaterial(BlockCanBuildEvent blockCanBuildEvent, @InjectPluginVersion ApiVersion version) {
        return transformFromBlockType(blockCanBuildEvent.getType(), version);
    }
    ...
    public static Material getMaterial(PlayerInteractEvent playerInteractEvent, @InjectPluginVersion ApiVersion version) {
        return transformFromItemType(playerInteractEvent.getItemType(), version);
    }
    ...
    public static Material getMaterial(PlayerStatisticIncrementEvent playerStatisticIncrementEvent, @InjectPluginVersion ApiVersion version) {
        if (playerStatisticIncrementEvent.getStatistic().getType() == Statistic.Type.BLOCK) {
            return transformFromBlockType(playerStatisticIncrementEvent.getBlockType(), version);
        } else if (playerStatisticIncrementEvent.getStatistic().getType() == Statistic.Type.ITEM) {
            return transformFromItemType(playerStatisticIncrementEvent.getItemType(), version);
        } else {
    ...
    public static List<Material> getChoices(RecipeChoice.MaterialChoice materialChoice, @InjectPluginVersion ApiVersion version) {
        return materialChoice.getChoices().stream().map(m -> transformFromItemType(m.asItemType(), version)).toList();
    }
    ...
    public static BlockData createBlockData(Material material) {
        return Bukkit.createBlockData(transformToBlockType(material).typed());
    }

    @RerouteStatic("org/bukkit/Bukkit")
    public static BlockData createBlockData(Material material, Consumer<? super BlockData> consumer) {
        return Bukkit.createBlockData(transformToBlockType(material).typed(), consumer);
    }

    @RerouteStatic("org/bukkit/Bukkit")
    public static BlockData createBlockData(Material material, String data) {
        return Bukkit.createBlockData(transformToBlockType(material).typed(), data);
    }
    ...
    public static BlockData createBlockData(Server server, Material material) {
        return server.createBlockData(transformToBlockType(material).typed());
    }

    public static BlockData createBlockData(Server server, Material material, Consumer<? super BlockData> consumer) {
        return server.createBlockData(transformToBlockType(material).typed(), consumer);
    }

    public static BlockData createBlockData(Server server, Material material, String data) {
        return server.createBlockData(transformToBlockType(material).typed(), data);
    }
    ...
    public static <T extends Keyed> Set<T> getValues(Tag<T> tag, @InjectPluginVersion ApiVersion version) {
        ...
        if (tag instanceof CraftBlockTag) {
            return values.stream().map(val -> (BlockType) val).map(val -> transformFromBlockType(val, version)).map(val -> (T) val).collect(Collectors.toSet());
        } else if (tag instanceof CraftItemTag) {
            return values.stream().map(val -> (ItemType) val).map(val -> transformFromItemType(val, version)).map(val -> (T) val).collect(Collectors.toSet());
        }
